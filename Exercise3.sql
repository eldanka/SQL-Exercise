...
SELECT AVG(Value) FROM Boxes Group by Contents;

--3.5 Select the warehouse code and the average value of the boxes in each warehouse.
SELECT AVG(Value),Warehouse FROM Boxes Group by Warehouse;

--3.6 Same as previous exercise, but select only those warehouses where the average value of the boxes is greater than 150.
SELECT Warehouse,AVG(Value) FROM Boxes  HAVING AVG(Value) >150 Group by Warehouse;
...
SELECT DISTINCT Warehouses.Code FROM Warehouses JOIN Boxes ON Warehouses.Code = Boxes.Warehouse HAVING Warehouses.Capacity <(SELECT SUM(Warehouse)FROM Boxes Group by Warehouse ) Group by Code
...
-- 3.18 Remove (drop) the index you added just
    -- !!!NOTE!!!: index should NOT be used on small tables in practice
DROP INDEX idx_warehouse ON Boxes(Warehouse);